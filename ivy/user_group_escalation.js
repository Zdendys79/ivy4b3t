...
export async function blockUserGroup(userId, groupId, reason) {
  try {
    ...
    // Logování
    await Log.warn(`[${userId}]`, `🚫 Skupina ${groupId} zablokována na ${blockDays} dní (${blockCount + 1}. problém)`);
    await Log.warn(`[${userId}]`, `📅 Dostupná opět: ${blockedUntil.toLocaleString()}`);
...
export async function detectMembershipRequest(user, group, pageContent) {
  ...
  if (detected) {
    ...
    if (blockResult.success) {
      await Log.warn(`[${user.id}]`, `⏱️ Skupina automaticky zablokována na ${blockResult.blockDays} dní`);
    }
