...
export class FBGroupAnalyzer {
  ...
  async analyzeCurrentGroup(userId) {
    try {
      ...
      const fbGroupId = await this.extractGroupIdFromUrl(currentUrl);
      if (!fbGroupId) {
   ...
   */
  async extractGroupIdFromUrl(url) {
    try {
      ...
      await Log.warn('[GROUP_ANALYZER]', `Chyba při extrakci group ID: ${err.message}`);
      return null;
    }
  }
  ...
  async collectGroupBasicInfo(fbGroupId) {
    try {
      ...
      await Log.warn('[GROUP_ANALYZER]', `Chyba při sběru základních info: ${err.message}`);
      return {
  ...
  async collectGroupAdvancedInfo() {
    try {
      ...
      await Log.warn('[GROUP_ANALYZER]', `Chyba při sběru pokročilých info: ${err.message}`);
      return {
        category: null,
        language: 'cs',
        activity_level: null,
  ...
  async saveGroupToDatabase(groupInfo, existingGroup) {
    try {
      ...
      await Log.error('[GROUP_ANALYZER]', `Chyba při ukládání do DB: ${err.message}`);
      throw err;
    }
  }
  ...
  async navigateToRandomGroup() {
    try {
      ...
      await Log.warn('[GROUP_ANALYZER]', `Chyba při navigaci: ${err.message}`);
      return false;
    }
  }
  ...
  async getUserExplorationStats(userId) {
    try {
      ...
      await Log.error('[GROUP_ANALYZER]', `Chyba při načítání statistik: ${err.message}`);
      return null;
    }
  }
}