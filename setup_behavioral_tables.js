...
async function setupBehavioralTables() {
  try {
    ...
    await Log.warn('[SECURITY]', '⚠️ DŮLEŽITÉ: Nikdy necommituj behavioral_profiles_schema.sql do Gitu!');
    await Log.warn('[SECURITY]', '⚠️ Obsahuje strukturu citlivých dat o uživatelích');

  } catch (error) {
    await Log.error('[SETUP]', `❌ Chyba během setup: ${error.message}`);
    process.exit(1);
  }
}
