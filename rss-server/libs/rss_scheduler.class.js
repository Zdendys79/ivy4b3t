...
export class RSSScheduler {
  ...
  async runRSSProcessing() {
    ...
    try {
      ...
      if (result.success) {
        const message = `RSS cycle #${this.runCount} completed: ${result.newUrls} new URLs from ${result.channel} (${duration}ms)`;
        ...
        const message = `RSS cycle #${this.runCount} failed: ${result.error} (${duration}ms)`;
      ...
      const duration = Date.now() - startTime;
      const message = `RSS cycle #${this.runCount} error: ${err.message} (${duration}ms)`;
      Log.error('[RSS_SCHEDULER]', message);
