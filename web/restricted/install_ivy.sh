...
for user in "${USERS[@]}"; do
  ...
  echo "1) Použít existující heslo"
  echo "2) Generovat nové náhodné heslo (12 znaků)"
  read -p "Volba (1/2): " choice
  
  case $choice in
    1)
      echo -n "Zadejte existující heslo pro uživatele '$user': "
      read -s USER_PASS
      echo
      ;;
    2)
      USER_PASS=$(node /home/remotes/ivy4b3t/scripts/enhanced-password-generator.js 40 0)
      echo "Vygenerované nové heslo pro '$user': $USER_PASS"
      ;;
    *)
      echo "Neplatná volba, generuji nové heslo."
      USER_PASS=$(node /home/remotes/ivy4b3t/scripts/enhanced-password-generator.js 40 0)
      echo "Vygenerované nové heslo pro '$user': $USER_PASS"
      ;;
  esac
