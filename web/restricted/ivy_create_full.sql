...
CREATE PROCEDURE `GetLockStatistics` (IN `days_back` int)
BEGIN
  ...
  SELECT
    ...
    COUNT(*) as value
  FROM fb_users
  WHERE locked > NOW() - INTERVAL 1 DAY
  ...
  SELECT
    ...
    COUNT(*) as value
  FROM fb_users
  WHERE locked > NOW() - INTERVAL 7 DAY;

  -- Statistiky podle typu
  SELECT
    COALESCE(lock_type, 'UNKNOWN') as lock_type,
    COUNT(*) as total_count,
    COUNT(CASE WHEN locked > NOW() - INTERVAL 1 DAY THEN 1 END) as last_24h,
    COUNT(CASE WHEN locked > NOW() - INTERVAL 7 DAY THEN 1 END) as last_7d,
    ROUND(AVG(TIMESTAMPDIFF(HOUR, locked, COALESCE(unlocked, NOW()))), 2) as avg_lock_duration_hours
  FROM account_lock_history
  WHERE locked_at > NOW() - INTERVAL days_back DAY
  GROUP BY lock_type
...
CREATE TRIGGER `account_lock_history_insert` AFTER UPDATE ON `fb_users` FOR EACH ROW
BEGIN
  -- Pokud se účet zablokoval (locked změněno z NULL na datetime)
  IF OLD.locked IS NULL AND NEW.locked IS NOT NULL THEN
    INSERT INTO account_lock_history (
      user_id, locked_at, lock_reason, lock_type, hostname
    ) VALUES (
      NEW.id, NEW.locked, NEW.lock_reason, NEW.lock_type, NEW.host
    );
  END IF;

  -- Pokud se účet odemkl (locked změněno z datetime na NULL)
  IF OLD.locked IS NOT NULL AND NEW.locked IS NULL THEN
...
CREATE TRIGGER `quotes_before_update` BEFORE UPDATE ON `quotes` FOR EACH ROW
BEGIN
    SET NEW.hash = MD5(NEW.text);
END;;
